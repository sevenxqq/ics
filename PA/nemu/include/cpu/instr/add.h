#ifndef __INSTR_ADD_H__
#define __INSTR_ADD_H__


make_instr_func(add_r2rm_b);    make_instr_func(add_r2rm_v);   make_instr_func(add_rm2r_b);   make_instr_func(add_rm2r_v);
make_instr_func(add_i2a_b);		make_instr_func(add_i2a_v);

make_instr_func(add_i2rm_v);
make_instr_func(add_i2rm_bv);
make_instr_func(add_i2rm_b);


make_instr_func(inc_r_v);
make_instr_func(inc_rm_v);


#endif
